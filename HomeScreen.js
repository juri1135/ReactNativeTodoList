...
function HomeScreen({ navigation }) {
  ...
  return (
    <ThemeProvider theme={appTheme}>
      ...
      {/* Header, TextInput, ScrollView */}
      <TouchableOpacity
        style={[styles.complete, { backgroundColor: appTheme === light ? light.blue : dark.grey }]}
        onPress={() => navigation.navigate('CompletedTasks😺', { todos, appTheme })}
      >
        <Text style={{ color: appTheme === light ? 'black' : 'white', fontSize: 15 }}>완료된 작업 (=^･ω･^=)</Text>
      </TouchableOpacity>
    </ThemeProvider>
  );
}
